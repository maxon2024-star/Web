...
* [Лекция 12. Адаптивная верстка. PWA. Tauri](lectures/Lecture_12_PWA.pdf)
...
Индивидуальные темы лабораторных 2026 года описаны в [файле](/topics.md) 
...
* [Методические указания FastAPI](/Web/tutorials/lab1-fastapi/FastAPI_ReadME.md)
* [Установка и администрирование Minio](/Web/tutorials/minio/MinIO_bucket_setup_ReadMe.md)
...
- **Порядок показа**: Сделать скриншоты всех изменений в РПЗ по 2026 году, скриншоты `TestVKR` и объема файла РПЗ.
...
**Отчет-РПЗ** по курсу не более 5 Мб:
...
2. **Предметная область**. Описание предметной области на примере конкретной компании/подразделения с названиями должностей и ссылками по теме, формула расчета значения по теме. Диаграмма прецедентов, диаграмма состояний и описания бизнес-процесса с диаграммой деятельности/BPMN (>500 слов)
...
### Дополнительные задания ДЗ

> Внести каждое в Задачи и Заключение

#### Индексы в БД (+4 балла)
...
* [Методические указания Пагинация и индексы](/tutorials/pagination_index/README.md)
* Полезные материалы [Индексы и план запроса](https://github.com/iu5git/Database/blob/main/tutorials/lab_index.md)

#### Создание мобильного приложения (+3 балла)
...
- **Задание**: Создание приложения для `гостя` на iOS/Android/Qt и подключение к веб-сервису
...
#### Межсервисное взаимодействие (+3 балла)
...
Требуется разработать второй простой асинхронный сервис на другом языке (кто делал на NestJS - Go и наоборот) с одним http-методом для выполнения отложенного действия в вашей системе (вычисление, моделирование, оплата и тд). Действие выполняется с задержкой 5-10 секунд, результат сервиса вычисляется по формулам. Достаточно в результате обновить одно поле по предметной области в `заявке`/`м-м`. При создании `заявки` это поле пустое.
...
* [Методические указания Asyncio](/tutorials/asyncio/README.md)


#### Темы методических указаний для детализации (+4 балла)
- Для доп. задания. Работа с индексами и пагинацией (в бэкенде и фронтенде): NextJS,  +4 балла
- Лабораторные работы 1-4 (каждая по 8 баллов) Fast API + Jinja2 + SQLAlchemy + Alembic+ куки
...
* Добавить новую куку с сессией гостя (не связанную с авторизацией), которая заполняется при посещении сайта и действует 20 минут с создания. При использовании на сервере проверяется ее время создания и при необходимости обновляется. Хранится в Redis. К этой сессии привязываются просмотренные `услуги`. Добавить в метод списка услуг фильтрацию по недавно просмотренным из `redis`, использовать этот метод на странице подробнее, где отображать список недавно просмотренных карточек
* Добавить новую таблица `media` c внешним ключом на таблицу `услуг`, в которой хранить ссылки не на одно изображение, а несколько файлов из `Minio` к данной услуге (фото и видео). Добавить метод удаления конкретного фото/видео по id из `media`. В странице `Подробнее` добавить `карусель`, порядок файлов расположить по id в таблице `media`. Каждое видео автоматически проигрывается при отображении. 
* Требуется выполнить дополнительное задание к ДЗ `Индексы в БД` но добавить формирование ленты наподобие `TikTok` в `Redis` для авторизованного пользователя. Лента хранится в `Redis` и формируется во время входа в приложение. Лента включает случайный набор 100 неповторяющихся карточек с порядковыми номерами для пользователя. Фото в `Minio` заменить на `видео`, которое автоматически проигрывается на странице `Подробнее` об `услуге`, на странице добавить кнопку `Далее`, по которой отображается следующая карточка. 
* Требуется добавить мультимодальный поиск в `React` в списке услуг с помощью `CLIP` по методическим указаниям 
* Для каждой услуги на странице подробнее на фронтенде формировать `QR код`, который можно отсканироват с телефона и открыть эту страницу в приложении. Локальную сеть настроить с помощью `ZeroTier`
* Реализовать иконку корзины как микрофронтенд на `svelte`. [Методические указания](/tutorials/svelte/README.md)
* Добавление агента с помощью `WebLLM` для `гостя` на строне браузера (Chrome, Яндекс, Arc) в приложение React для формирования параметров поиска на странице `Списка услуг`: необходимо подобрать промпт для LLM, с помощью которого она будет формировать `JSON` для параметров поиска из текстового запроса пользователя по теме-варианту. Этот `JSON` используется параметров в запросе к бэкенду. [Методические указания WebLLM](/tutorials/WebLLM/README.md) 
* Выполнить задание `Интерфейс модератора`
...
1. Перечислите основные составляющие web и опишите их.
2. Что такое HTML, CSS? Приведите примеры.
3. Опишите шаблон MVC, структуру и назначение компонентов.
4. Что такое чистая архитектура?
5. Что такое MVC фреймворк? Его назначение и возможности.
6. Что такое шаблонизация? Приведите примеры.
7. Виды баз данных. Приведите примеры и отличия.
8. Объясните назначение ORM, ее составляющие. Укажите преимущества и недостатки ORM.
9. Что такое модель и миграция?
10. Укажите группы SQL запросов, их примеры и назначение.
11. Что такое курсор? Преимущества и пример.
12. Отличие продукта и процесса. Что такое эпики и дорожная карта?
13. Доска Kanban и пользовательские истории.
14. Перечислите основные диаграммы UML и их назначение.
15. Методология разработки Agile. Состав IT команды.
16. Что такое git? Опишите схему работы с ветками GitHub.
17. Опишите протокол HTTP. Схему работы и основные понятия.
18. Опишите стек протоколов интернета TCP/IP.
19. Что такое URI? Опишите элементы URI для HTTP.
20. Протокол HTTPS, схема, назначение ключей.
21. Что такое веб-сервис? Отличие от веб-сервера.
22. Что такое Web API? Назначение и применение.
23. Микросервисная архитектура. Отличия от монолитной архитектуры.
24. Перечислите требования REST, опишите их.
25. Что такое RPC? Варианты RPC и их отличия.
26. Назначение JSON и XML. Примеры и отличия.
27. Что такое Redis? Его назначение и варианты применения.
28. Опишите схему авторизации с помощью JWT.
29. Опишите схему авторизации с помощью сессий.
30. Что такое авторизация и аутентификация? Укажите варианты авторизации и их отличия.
31. Что такое Swagger? Назначение и использование.
32. Что такое алгоритм RSA? Опишите работу алгоритма.
33. Двухфакторная аутентификация
34. Что такое SSO? Схема работы.
35. Протокол OAuth. Схема работы.
36. Что такое React, схема работы виртуального DOM?
37. Что такое компонент, его состояния и свойства.
38. JSX что это? Пример и особенности.
39. Структура React проекта. Назначение Babel и Vite.
40. Жизненный цикл React компонента.
41. Назначение хуков useState и useEffect.
42. Что такое Cors? Укажите варианты решения.
43. Что такое WebLLM?
44. Отличия GraphQL, API Gateway.
45. Архитектура FSD, преимущества, назначение составляющих архитектуры.
46. Назначение хуков useContext и useReducer.
47. Опишите схему работы менеджера состояний Redux с примерами кода.
48. Какие параметры передаются при создании Store? Их примеры и назначение.
49. Что такое Redux Thunk, пример кода, схема.
50. Что такое AJAX? Схема работы и назначение.
51. Перечислите отличия Axios от fetch. Приведите примеры.
52. Опишите генерацию JS-кода взаимодействия с API на основе описания Swagger. 
53. Опишите этапы подхода DevOps. Назначение GitHub Pages.
54. Что такое pwa? Отличия от других вариантов приложений.
55. Назначение фреймворков Electron и Tauri. Их отличия.
56. Отличия мобильных и веб-приложений. Языки и технологии для разработки мобильных приложений.
57. Плюсы и минусы разработки на React Native.
58. Опишите плюсы и минусы реализации асинхронности и пути их преодоления.
59. Что такое Web реального времени? Что такое WebSocket?
60. Что такое DMZ? Опишите на примере системы разделение ее компонентов на подсети.

#### Темы индивидуальных заданий  (назначается преподавателем)

> кому на ОТЛ не хватило дополнительных заданий, +4 балла
